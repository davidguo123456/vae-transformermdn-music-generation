...
def get_dataset(dataset='',
  ...
  if problem == 'mnist':
    ...
    train_ds = data_utils.get_tf_record_dataset(
        ...
        shuffle=True,
        tokens=tokens)
    test_ds = data_utils.get_tf_record_dataset(
  ...
  test_ds = test_ds.map(partial(deconstruct_dict, problem=problem),
                        num_parallel_calls=AUTOTUNE)
  ...
  if problem != 'tokens':
    train_ds = train_ds.map(lambda example: tf.py_function(
        partial(data_transform, problem=problem, pca=pca), [example], tf.float32
                            ...
                            num_parallel_calls=AUTOTUNE)
    test_ds = test_ds.map(lambda example: tf.py_function(
        partial(data_transform, problem=problem, pca=pca), [example], tf.float32
    ),
                          num_parallel_calls=AUTOTUNE)
    ...
    train_ds = train_ds.map(partial(slice_transform,
                            ...
                            num_parallel_calls=AUTOTUNE)
    test_ds = test_ds.map(partial(slice_transform,
                                  problem=problem,
                                  slice_idx=slice_idx,
                                  dim_weights=dim_weights),
                          num_parallel_calls=AUTOTUNE)
